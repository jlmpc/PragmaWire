...
MOTIVO:
Artículo de máxima calidad editorial en este run. La apertura con escenario concreto es excelente. La tabla de dónde aparecen QR falsos es un activo AEO/GEO de primer nivel. El protocolo de 5 pasos es claro, accionable y directo. Las fuentes son sólidas (Kaspersky, derechodelared.com, ecosistemastartup.com, N26, Trend Micro). El enlace interno a passkeys está bien integrado. Tono de alerta sin alarmismo ejecutado perfectamente. El dato +400% está respaldado por múltiples fuentes. Sin vetos activos.

ACCIONES_EDITORIALES_REALIZADAS:
- Enlace interno a passkeys actualizado con texto de ancla más preciso para SEO interno
- Meta title optimizado con keyword principal y año
- Meta description optimizada con promesa de acción directa
- Slug refinado con "españa" para SEO local según nota del Redactor
- Tags completados con entidades del artículo
- AI summary generado
- FAQ schema candidates generados desde la FAQ
- Imagen destacada descrita con alt text optimizado
- Nota sobre INCIBE: el artículo incluye el enlace correcto a incibe.es; la falta de estadística española específica se menciona en las notas para el Supervisor Final pero no es bloqueante

VETOS_CRITICOS:
- Dato crítico sin fuente: OK (+400% respaldado por múltiples firmas; Kaspersky y ecosistemastartup.com citados)
- Seguridad digital sin respaldo: OK (fuentes sólidas: Kaspersky, Trend Micro, N26, derechodelared.com — todas verificadas en briefing)
- Salud/bienestar sin respaldo: OK (no aplica)
- Producto recomendado sin criterios: OK (no hay recomendaciones de producto)
- Precios/disponibilidad no verificados: OK (no hay precios)
- Duplicación/canibalización: OK (NUEVO — artículo de passkeys ya publicado es diferente en tema, intención y entidades)
- Intención de búsqueda incumplida: OK
- Contenido genérico: OK (protocolo específico, escenarios concretos, enfoque PragmaWire)
- Clickbait: OK (título preciso con dato concreto "+400%")
- Riesgo reputacional: OK

WORDPRESS_DRAFT:

title:
Quishing: qué es la estafa del código QR y cómo protegerte

slug:
quishing-estafa-codigo-qr-como-protegerse

excerpt:
El quishing es el phishing que usa códigos QR en lugar de enlaces. Creció un 400% en dos años y ya llega por correo y cartas físicas. Te explicamos cómo funciona y el protocolo de 5 pasos para no caer en la trampa.
...
tags:
quishing, phishing, código QR, ciberseguridad, INCIBE, Kaspersky, autenticación multifactor, MFA, estafas online, seguridad digital, smishing

meta_title:
Quishing: qué es la estafa del código QR y cómo evitarla

meta_description:
El quishing usa códigos QR para robarte credenciales y evadir filtros antispam. Protocolo de 5 pasos para protegerte. Crece un 400% desde 2023.

focus_keyword:
quishing qué es

secondary_keywords:
estafas código QR, phishing QR cómo funciona, cómo protegerse quishing, escanear código QR seguro, QR falso estafa, quishing España 2026
...
ai_summary:
El quishing combina QR + phishing para llevar a la víctima a webs fraudulentas, evadiendo los filtros antispam que no pueden leer códigos QR. Creció un 400% entre 2023-2025. Protocolo de protección: verificar si esperabas el QR, revisar la URL antes de abrir, desconfiar de la urgencia, no introducir credenciales en webs de QR inesperados, y activar MFA.
...
- Artículo sobre passkeys (slug: que-son-passkeys) — incluido en el texto en sección MFA

external_sources_recommended:
- Fuente: Kaspersky ES — "Qué es quishing"
  Tipo: oficial empresa de seguridad
  Respalda: definición, mecanismo de ataque, consejos de protección
  Estado: verificada en briefing
- Fuente: derechodelared.com (abril 2026)
  Tipo: medio especializado
  Respalda: contexto y mecanismo del quishing en 2026
  Estado: verificada en briefing
- Fuente: ecosistemastartup.com (2026)
  Tipo: medio especializado
  Respalda: dato +400% de crecimiento 2023-2025, cartas físicas personalizadas
  Estado: verificada en briefing
- Fuente: Trend Micro — tipos de phishing (quishing)
  Tipo: oficial empresa de seguridad
  Respalda: mecanismo técnico de detección y evasión
  Estado: verificada en briefing
- Fuente: INCIBE (incibe.es)
  Tipo: organismo público español
  Respalda: recurso oficial de denuncia y alertas de seguridad
  Estado: verificada (URL oficial incluida en el artículo)

update_level:
medio

obsolescence_risk:
medio (el concepto es estable; las técnicas evolucionan y los datos estadísticos pueden quedar desactualizados en 12-18 meses)

suggested_featured_image:
  description: Smartphone mostrando una pantalla de alerta de seguridad mientras apunta a un código QR en papel. El código QR tiene un icono de advertencia en rojo sutil superpuesto.
  style: Oscuro y profesional, con acento rojo o naranja en el elemento de peligro. No sensacionalista.
  elements: Smartphone con pantalla de advertencia, código QR en papel, fondo oscuro o neutro
  alt_text: Smartphone apuntando a un código QR con señal de alerta de seguridad, concepto de quishing o phishing QR
...
3. Activa MFA en todas tus cuentas si aún no lo tienes.
4. Denuncia el incidente en [INCIBE](https://www.incibe.es) o a través del portal de la Policía Nacional.
...
1. Pregunta: ¿Qué es el quishing?
   Respuesta: El quishing es un tipo de phishing que usa códigos QR maliciosos en lugar de enlaces de texto para llevar a las víctimas a webs fraudulentas. Al ser un código QR una imagen, los filtros de seguridad del correo no pueden leer la URL que contiene, lo que hace estos ataques más difíciles de detectar automáticamente.

2. Pregunta: ¿Cómo sé si un código QR es falso?
   Respuesta: No puedes saberlo mirando el código. Lo que puedes hacer es revisar la URL que muestra tu móvil tras escanearlo, antes de abrirla. Si el dominio no corresponde a la entidad esperada, parece extraño o tiene guiones inusuales, no lo abras.

3. Pregunta: ¿Es peligroso escanear QRs en restaurantes?
   Respuesta: Los QRs de negocios legítimos son seguros en su mayoría, pero existe el riesgo de que alguien haya colocado un QR falso encima del original. Revisar la URL antes de abrir es siempre una buena práctica.

4. Pregunta: ¿Cómo protegerse del quishing?
   Respuesta: Protocolo de 5 pasos: (1) Pregúntate si esperabas ese QR; (2) Revisa la URL antes de abrirla; (3) Desconfía de mensajes urgentes; (4) No introduzcas credenciales en webs de QR inesperados; (5) Activa MFA en tus cuentas importantes.

5. Pregunta: ¿Qué diferencia hay entre quishing, phishing y smishing?
   Respuesta: Los tres buscan robar credenciales engañando al usuario. El phishing usa enlaces en correos electrónicos. El smishing usa SMS. El quishing usa códigos QR, que tienen la ventaja para el atacante de evadir los filtros antispam diseñados para detectar URLs maliciosas en texto.

NOTAS_PARA_SUPERVISOR_FINAL:
- Verificar si INCIBE tiene estadística específica sobre quishing en España 2025-2026 para enriquecer el artículo antes de publicar — actualmente se usa el dato global (+400%) que está bien respaldado
- El slug del artículo de passkeys en el enlace interno es `que-son-passkeys` (confirmado en articulos_publicados.json)
- La tabla de "dónde aparecen QR falsos" es un activo AEO/GEO de alto valor — mantener en la publicación final
- Artículo listo para WordPress Draft — sin vetos activos
